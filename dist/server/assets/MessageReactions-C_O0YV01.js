import { c as createLucideIcon, u as useAuth, s as supabase } from "./router-CYx1i6QQ.js";
import { t as toDate, h as constructFrom, c as getDefaultOptions, n as normalizeDates, g as getTimezoneOffsetInMilliseconds, i as millisecondsInDay, j as millisecondsInWeek, e as enUS, f as constructNow } from "./en-US-CqQV4g4D.js";
...
const QUICK = ["👍", "❤️", "😂", "🔥", "😮", "🎉"];
function MessageReactions({ messageId, scope, align = "left" }) {
  const { user } = useAuth();
  ...
  reactExports.useEffect(() => {
    ...
    const ch = supabase.channel(`react:${scope}:${messageId}`).on("postgres_changes", { event: "*", schema: "public", table, filter: `message_id=eq.${messageId}` }, async () => {
      const { data } = await supabase.from(table).select("message_id, user_id, emoji").eq("message_id", messageId);
      setRows(data ?? []);
    }).subscribe();
    return () => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `flex flex-wrap items-center gap-1 mt-1 ${align === "right" ? "justify-end" : ""}`, children: [
    grouped.map(([emoji, info]) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        children: [
          emoji,
          " ",
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-semibold", children: info.count })
        ]
      },
      emoji
    )),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative", children: [
      ...
      open && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute z-20 bottom-full mb-1 left-0 flex gap-1 p-1.5 rounded-full bg-card border border-border shadow-lg animate-in fade-in zoom-in-95", children: QUICK.map((e) => /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => toggle(e), className: "size-7 grid place-items-center rounded-full hover:bg-secondary text-base", children: e }, e)) })
    ] })
  ] });
}
export {
  MessageReactions as M,
  Pencil as P,
  Send as S,
  Trash2 as T,
  isYesterday as a,
